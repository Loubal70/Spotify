        ...
        <div class="card-body">
          ...
          <form action="{{ route('playlistcreate', app()->getLocale()) }}" method="POST" enctype="multipart/form-data" data-pjax>
            <h1>{{ __('Créer une playlist') }}</h1>
              ...
              <div class="form-group row align-items-center">
                <label class="col-md-4 col-form-label text-md-right text-white" for="imgMusiq"> {{ __('Image de votre playlist') }} :</label>
                <div class="col-md-6">
                  <input type="file" id="imgMusiq" name="imgMusiq" value="{{old('url')}}" class="form-control" required>
                </div>
              </div>

              <input type="submit" class="btn btn-primary d-block ml-auto" value="{{ __('Envoyer') }}">
